...
test("transform", function() {
  ...
  const serializeBox = createTransformer((node: typeof Node) => getSnapshot(node), {isObservable: false})

  const serializeArrow = createTransformer((edge: typeof Edge) => getSnapshot(edge), {isObservable: false})
