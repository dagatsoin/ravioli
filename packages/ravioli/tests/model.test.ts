...
/*  }))
  .addActions({
    /**
     * An action is an abstraction of the mutations mechanism.
     * A mutation is a low level operation on the model and an action is a higher level operation, hiding the
     * complexity of the mutations to the container representation (eg. the view).
     * 
     * An action has one responsability: present a list of mutations needed for the model to accomplish the action.
     * 
     * To achieve this an action can apply context related code:
     * - write payload validation rules (eg: validate a password format, sanitize string to prevent code injection)
     * - map allowed actions to control states
     * - map mutations to actions
     *
     * An action is not aware of the internal state of the model. But it
     * knows about the context this action is used.
     * The context of an action is all its dependencies it may called to. Example: upper model state in
     * case of nested component, global services, etc.)
     * 
     * An action must be pure and indenpotent. That means that all those dependencies must be also indenpotent.
     * Good examples:
     *  1. is this password well formatted?
     *    No deps, just a hard coded business rule.
     *  2. is this string is a code injection attempt?
     *    No deps too, just a hard coded security rule.
     *  3. can the player launch a fire ball?
     *    Trickier here. Needed dependency:
     *      - some business rules
     *      - the container model
     * Bad examples:
     ...
     *    and old passwords are part of the past.
     *
